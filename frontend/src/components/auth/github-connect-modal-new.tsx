...
export function GitHubConnectModal() {
  ...
  useEffect(() => {
    ...
    if (prompt === 'connect-github') return
    if (location.pathname?.startsWith('/connect-github')) return
    ...
    if (
      connectedFlag === '1' ||
      document.cookie.includes('gitguard_github_connected=1') ||
      hasGitHub
    ) {
      setOpen(false)
      return
  ...
  }, [hasGitHub, isLoaded, isSignedIn, location.pathname, prompt, user])
      ...
      <div className="relative w-full max-w-xl overflow-hidden rounded-2xl border border-blue-100/20 bg-card shadow-2xl">
        ...
        <div className="border-b bg-gradient-to-r from-primary/20 via-blue-600/10 to-amber-500/10 p-6">
          ...
          <p className="mt-2 text-sm text-muted-foreground">
            Connect your professional GitHub account now to sync repositories and run AI reviews.
          </p>
        </div>

        <div className="space-y-4 p-6">
          <div className="rounded-xl border border-border/70 bg-muted/30 p-4 text-sm text-muted-foreground">
            You will be asked to authorize professional GitHub access, similar to the standard GitHub
            consent screen.
          </div>

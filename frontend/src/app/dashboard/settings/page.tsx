...
export default function SettingsPage() {
    ...
    const handleNumberChange = (key: "llmTemperature" | "maxDiffSize" | "reviewDelay", value: number) => {
        updateSettings({ [key]: value })
    }
            ...
            <div className="flex items-center justify-between">
                <div>
                    ...
                    <p className="text-muted-foreground mt-1">
                        Configure your GitGuard AI preferences
                    </p>
                </div>
                <button
                    onClick={() => {
                        ...
                        <div className="flex gap-2">
                            {(["low", "medium", "high", "critical"] as const).map((level) => (
                                <button
                                    ...
                                    className={`px-4 py-2 rounded-lg text-sm font-medium transition-colors ${settings.severityThreshold === level
                    ...
                    <CardContent className="p-6">
                        <div className="flex items-center justify-between">
                            ...
                            <button
                                ...
                                className={`relative inline-flex h-6 w-11 items-center rounded-full transition-colors ${settings.autoComments ? "bg-primary" : "bg-muted-foreground/30"
                                    }`}
                            >
                                <span
                                    className={`inline-block h-4 w-4 transform rounded-full bg-white transition-transform ${settings.autoComments ? "translate-x-6" : "translate-x-1"
                                        }`}
                                />
                            </button>
                        </div>
                    ...
                    <CardContent className="p-6">
                        <div className="flex items-center justify-between">
                            ...
                            <button
                                ...
                                className={`relative inline-flex h-6 w-11 items-center rounded-full transition-colors ${settings.autoFixes ? "bg-primary" : "bg-muted-foreground/30"
                                    }`}
                            >
                                <span
                                    className={`inline-block h-4 w-4 transform rounded-full bg-white transition-transform ${settings.autoFixes ? "translate-x-6" : "translate-x-1"
                                        }`}
                                />
                            </button>
                        </div>

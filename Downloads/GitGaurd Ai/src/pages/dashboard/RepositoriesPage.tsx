...
export default function RepositoriesPage() {
  ...
  const handleToggleSecurity = async (id: string) => {
    ...
    if (repo) {
      ...
      toast.success(
        `Security scan ${!repo.securityScan ? 'enabled' : 'disabled'} for ${repo.name}`
      )
    }
  }
  ...
  const handleToggleIgnoreStyling = async (id: string) => {
    ...
    if (repo) {
      ...
      toast.success(
        `Ignore styling ${!repo.ignoreLint ? 'enabled' : 'disabled'} for ${repo.name}`
      )
    }
  }
                ...
                <CardContent className="p-6">
                    ...
                    <div className="flex items-center gap-4">
                      <div
                        className={`p-3 rounded-lg ${
                          repo.status === 'active' ? 'bg-green-500/10' : 'bg-yellow-500/10'
                        }`}
                      >
                        <GitBranch
                          className={`h-6 w-6 ${
                            repo.status === 'active' ? 'text-green-500' : 'text-yellow-500'
                          }`}
                        />
                      </div>
                      <div>
                  ...
                  <div className="flex flex-wrap gap-4 mt-4 pt-4 border-t">
                    ...
                    >
                      ...
                      <div
                        className={`w-4 h-4 rounded-full ${
                          repo.strictMode ? 'bg-primary' : 'bg-muted-foreground/50'
                        }`}
                      />
                    </button>
                    ...
                    <div className="flex items-center gap-2 rounded-lg bg-muted px-3 py-1.5 text-sm text-muted-foreground">
                      ...
                      <span>
                        {repo.hasWorkflows ? 'CI workflow detected' : 'No CI workflow'}
                      </span>
                    </div>
                  </div>

...
const navItems = [
  ...
  { name: 'Review History', href: '/dashboard/review-history', icon: History },
...
export function Sidebar({ open, onToggle }: SidebarProps) {
  ...
  return (
    <motion.aside
      ...
      className="relative h-screen bg-card border-r flex flex-col shadow-xl lg:shadow-none"
    >
      {/* Close button for mobile */}
      <button
        onClick={onToggle}
        className="absolute top-4 right-4 p-1.5 rounded-lg hover:bg-accent transition-colors lg:hidden z-10"
        ...
        <AnimatePresence mode="wait">
          {open && (
            ...
            >
              ...
              <p className="text-xs text-muted-foreground mt-1">Unlimited PR reviews</p>
              ...
              <p className="text-xs text-muted-foreground mt-1">750 / 1000 PRs this month</p>
            </motion.div>
          )}
        </AnimatePresence>
      </div>
